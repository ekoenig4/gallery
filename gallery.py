...
def ProcessDir(indir, is_parent=True, subdirs=[]):
    ...
    for f, all_exts in sorted(base_files.items()):
        ...
        if args.verbose:
            print(info)
        elements += PLOT_ELE.format(file='%s.png' %
                                    f, tags=tags, extra=extra, fontsize=args.title_size)
        ...
        for f, all_exts in sorted(other_files.items()):
            for ext in all_exts:
                ...
                if ext == '.md':
                    ...
                    other_html.append(
                        '  <li><a href="{FILEHTML}">{FILE}</a> <a href="{FILE}">[raw]</a></li>'.format(FILEHTML=f+'.html', FILE=f+ext))
                else:
                    other_html.append(
    ...
    if not is_parent or len(subdirs) > 0:
        links += '<p><strong>Current directory:</strong> %s</p>' % indir

        links += '<p><strong>Navigation:</strong> '
        if not is_parent:
            links += ' <a href="../index.html">[Up]</a></p>'
        else:
            links += ' </p>'

        for i, subdir in enumerate(subdirs):
            links += '<p><a href="{SUBDIR}/index.html">{SUBDIR}</a></p>'.format(
                SUBDIR=subdir)
